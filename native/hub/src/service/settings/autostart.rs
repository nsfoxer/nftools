...
use crate::messages::common::BoolMsg;
...
impl ImmService for AutoStartService {
    async fn handle(&self, func: &str, req_data: Vec<u8>) -> Result<Option<Vec<u8>>> {
        ...
        func_typeno!(self, func, req_data, set_autostart, BoolMsg);
        func_end!(func)
    }
}
...
impl AutoStartService {
    ...
    /// 获取是否已启用开机自启动
    fn get_autostart(&self) -> Result<BoolMsg> {
        let r = self.auto_launch.is_enabled()?;
        Ok(BoolMsg{value: r})
    }

    /// 启用或禁用开机自启动
    fn set_autostart(&self, value: BoolMsg) -> Result<()> {
        if value.value {
